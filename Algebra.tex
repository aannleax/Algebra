...
\usepackage{mathtools}
\newcommand\filledcirc{\mathop{{\color{black}\bullet}\mathllap{\circ}}}
...
\begin{theorem}
  ...
  \begin{equation*}
    f_{\text{row}(\alpha)} \filledcirc f_{\text{row}(\beta)} = f_{\text{row}(\alpha \otimes \beta)}.
  \end{equation*}
\end{theorem}
\begin{proof}
...
\begin{definition}
  ...
  bestehend aus einem Summoid $\mathcal{A} = (A, \Sigmacirc, \circledast, a)$,
  sodass $(\mathcal{A}_\text{Sum}, \mathcal{S}, \text{scal})$ ein Summoid-Modul bildet,
  wobei für alle $s \in S$ und $x, y \in A$
  \begin{equation*}
    s \cdot (x \circledast y) = (s \cdot x) \circledast (s \cdot y).
  \end{equation*}
\end{definition}
...
\begin{theorem}
  ...
  $\mathcal{A} = (A, \Sigmacirc, \circledast^A, a)$ eine $\mathcal{S}$-Algebra.
...
\begin{proof}
  \begin{align*}
    ...
    &= \sumcirc_{m \in M} \sumcirc_{c + d = m} (u(c) \ast w(d)) \cdot (\kappa(c) \circledast^\mathcal{A} \kappa(d)) \\
    &= \sumcirc_{m \in M} \sumcirc_{c + d = m} (u(c) \cdot \kappa(c)) \circledast^\mathcal{A} (w(d) \cdot \kappa(d)) \\
    &= \sumcirc_{(c, d) \in M \times M} (u(c) \cdot \kappa(c)) \circledast^\mathcal{A} (w(d) \cdot \kappa(d)) \\
    &= \sumcirc_{c \in M} u(c) \cdot \kappa(c) \circledast^\mathcal{A} \sumcirc_{d \in M}w(d) \cdot \kappa(d) \\
    &= (u \odot \kappa) \circledast^\mathcal{A} (w \odot\kappa).
  \end{align*}
...
\begin{example}
  ...
  ein Monoid-Homomorphismus von $\mathbb{N}$ nach $\mathcal{S}_\text{Mult}$.
  Dann ist 
  \begin{equation*}
    f_\kappa \colon S^\mathbb{N} \to S, u \mapsto \sum_{n \in \mathbb{N}}u(n) \cdot \kappa(n)
  \end{equation*}
  ein Summoid-Homomorphismus von $\mathcal{S}[\mathbb{N}_\text{add}]$ nach $\mathbb{S}$.
  Das heißt folgendes: Für $u \in S^\mathbb{N}$, welcher als Polynom
  \begin{equation*}
...
\begin{definition}
  ...
  Dann ist
  \begin{equation*}
    \ostar \colon \mathcal{P}(E) \times \mathcal{P}(E), (A, B) \mapsto \{a \star b \mid (a, b) \in E^{\langle 2 \rangle} \cap A \times B\}
  \end{equation*}
...
\begin{definition}
  Sei $\mathbb{G} = (G, \star, \text{id})$ mit $G = (V, E, \varrho)$ ein Aktionsnetzwerk.
  Dann ist $\text{Minsk}(\mathbb{G}) = (\mathcal{P}(E), \bigcup, \ostar, \text{id}(V))$
  ein Join-Summoid, welches wir das Minkowski-Summoid zu $\mathbb{G}$ nennen.
\end{definition}

\begin{theorem}
  Sei $\mathbb{G} = (G, \star, \text{id})$ mit $G = (V, E, \varrho)$ ein Aktionsnetzwerk.
  Dann ist das Minkowski-Summoid $\text{Minsk}(\mathbb{G}) = (\mathcal{P}(E), \bigcup, \ostar, \text{id}(V))$
  ein Join-Summmoid.
\end{theorem}
\begin{proof}
  ???
...
\begin{example}
  ...
  d.h.
  \begin{equation*}
    A \ostar B = \{a + b \mid a \in A, b \in B\}.
  \end{equation*}
\end{example}
...
\begin{definition}
  ...
  \begin{itemize}
    \item $A^{(0)} = \text{id}(V)$
    \item $A^{(1)} = A$
    \item $A^{(n + 1)} = A^{(n)} \ostar A$ für alle $n \in \mathbb{N}_{\geq 1}$
  \end{itemize}
...
\begin{example}
  ...
  \begin{align*}
    A^{(0)} &= \{0\} \\
    A^{(1)} &= A = \{2, 5\} \\
    A^{(2)} &= A \ostar A = \{2+2, 5+2, 2+5, 5+5\} = \{4, 7, 10\} \\
    A^{(3)} &= A^{(2)} \ostar A = \{4+2, 4+5, 7+2, 7+5, 10+2, 10+5\} = \{6, 9, 12, 15\} \\
    \dots \\
...
\begin{definition}
  ...
  falls $\delta^P_p \circledast \kappa(w) = \delta^P_q$,
  d.h. $(\kappa(w))(p, q) = 1$
  ???
\end{definition}
